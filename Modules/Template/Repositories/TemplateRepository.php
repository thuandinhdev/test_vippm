...
class TemplateRepository
{
	...
	public function create($request)
	{
		...
		DB::beginTransaction();
		try {
			$template->fill(["machine_name" => $this->webHelper->stripVN($input['name'])]);
			if ($template->save()) {
				...
				// if (!file_exists(public_path('themes/'.$themeActive['folder'].'/'.$input['folder']))) {
				// 	mkdir(public_path('themes/'.$themeActive['folder'].'/'.$input['folder']), 0777, true);
				// }
	...
	{
		$template = Template::join('web_templates_translation', 'web_templates_translation.templates_id', '=', 'web_templates.id')->where('web_templates_translation.langcode', 'vi')->where('web_templates.id', $id)->select('web_templates.id', 'web_templates.*',  'web_templates_translation.name', 'web_templates_translation.description')->first();

		if ($template) {
			// $template['content_html'] = file_get_contents(resource_path($template['content_html']));
			return $template;
		}
	...
	public function update($request, $id)
	{
		...
		try {
			...
			// 	if (file_exists(public_path('themes/'.$themeActive['folder'].'/'.$oldData['folder']))){
			// 		rename (public_path('themes/'.$themeActive['folder'].'/'.$oldData['folder']), public_path('themes/'.$themeActive['folder'].'/'.$input['folder']));
			// 	} else {
			// 		mkdir(public_path('themes/'.$themeActive['folder'].'/'.$input['folder']), 0777, true);
			// 	}
			// }
			...
			// File::put(resource_path($url), $input['content_html']);
			// $input['content_html'] = $url;
			$template->fill(['id'=>$input['id'], 'machine_name'=>$input['machine_name'] ]);
			if ($template->save()) {

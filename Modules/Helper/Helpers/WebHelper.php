...
class WebHelper
{
	...
	public function buildTemplate($template_id){
		...
		foreach ($data as $value){
			$html = file_get_contents(public_path('themes/'.$themeActive['folder'].$value->content_html));
		...
		$pageDisplay = file_get_contents(public_path('themes/'.$themeActive['folder'].'/page.html'));
	...
	public function buildPage($page_id){
		...
		foreach ($template as $value){
			if(file_exists(public_path('themes/'.$themeActive['folder'].$value->content_html))){
				$regionHtml = file_get_contents(public_path('themes/'.$themeActive['folder'].$value->content_html));
				for ($x = 0; $x < $value->number_content; $x++) {
					...
					foreach ($widgetData as $item){
						if(file_exists((public_path('themes/'.$themeActive['folder'].$item->content_html)))){
							$multipleData = [];
							...
							if($check_data_valid){
								$widgetHtml .= file_get_contents(public_path('themes/'.$themeActive['folder'].$item->content_html));
							} else {
	...
	public function _uploadImage($imageData, $path,$stl = false)
	{
	...
	public function builControlPage(){
		$pageData = DB::table('web_pages')->join('web_pages_translation', 'web_pages.id', '=', 'web_pages_translation.pages_id')->where('web_pages_translation.langcode', 'vi')->select('*', 'web_pages.id as id')->first();
		if(!$pageData){
			return;
		}
		$template = DB::table('web_template_region')->join('web_regions', 'web_regions.id', '=', 'web_template_region.regions_id')->join('web_regions_translation', 'web_regions.id', '=', 'web_regions_translation.regions_id')
				...
				for ($x = 0; $x < $value->number_content; $x++) {
					...
					foreach ($widgetData as $item){
						if(file_exists((public_path('themes/'.$themeActive['folder'].$item->content_html)))){
							...
							if($check_data_valid){
								$widgetHtml .= "<div onclick='callbackEditWidget(".$item->id.")'>".file_get_contents(public_path('themes/'.$themeActive['folder'].$item->content_html))."</div>";
							} else {
								$widgetHtml .= "<div onclick='callbackEditWidget(".$item->id.")'><h2>Không tìm thấy dữ liệu</></div>";
							}
							
							foreach($custom_fieldData as $keyCTdata => $itemCTdata){
								// dd([$item->machine_name, $keyCTdata, $itemCTdata, '{{$'.$item->machine_name."['".$keyCTdata."']".'}}']);
								$widgetHtml = str_replace('{{$'.$item->machine_name."['".$keyCTdata."']".'}}',  $itemCTdata , $widgetHtml);
								
							}
							// $widgetHtml .= "</div>";
							
							// $widgetHtml = "<div style='border: 5px solid red;padding:5px;'>".$widgetHtml."</div>";
						}
					}
					$regionHtml = str_replace('[content'.$x.']', $widgetHtml, $regionHtml);
				}	
		...
		$resource = str_replace('[content]', $regionsData, $pageDisplay);
		$resource = str_replace('{{$resource_path}}', $APP_URL.'../resources/themes/'.$themeActive->folder.'/', $resource);
		$resource_path = $APP_URL.'../resources/themes/'.$themeActive->folder.'/';
		$public_path = $APP_URL;
		$file_path = $APP_URL.'uploads/project/';
		$resource = str_replace('{{$resource_path}}', $resource_path, $resource);
		$resource = str_replace('{{$public_path}}', $public_path, $resource);
		$resource = str_replace('{{$file_path}}', $file_path, $resource);

		File::put(resource_path('controlpage/control-page.php'), $resource);
		return 'http://localhost/web-food-demo/resources/controlpage/control-page.php';

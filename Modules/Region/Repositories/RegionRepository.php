...
class RegionRepository
{
	...
	public function create($request)
	{
		...
		if (!file_exists(public_path('themes/'.$themeActive['folder'].'/regions'))) {
			mkdir(public_path('themes/'.$themeActive['folder'].'/regions'), 0777, true);
		}
	...
	public function update($request, $id)
	{
		...
		if($input['machine_name'] != $oldData['machine_name']){
			rename (public_path('themes/'.$themeActive['folder'].'/regions/'.$oldData['machine_name'].'.html'), public_path('themes/'.$themeActive['folder'].'/regions/'.$input['machine_name'].'.html'));
		}
		$url = '/regions/'.$input['machine_name'].'.html';
		File::put(public_path('themes/'.$themeActive['folder'].$url), $input['content_html']);
		$input['content_html'] = $url;
	...
	public function delete($id)
	{
		...
		try {
	        ...
	        // $themeActive = $this->webHelper->getThemeActive();
	        // unlink(public_path('themes/'.$themeActive['folder'].$region['content_html']));
	        DB::commit();
            return true;
        } catch (\Exception $e) {
		    DB::rollback();

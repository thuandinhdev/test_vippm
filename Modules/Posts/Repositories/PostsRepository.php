...
class PostsRepository
{
	...
	public function findById($id)
	{
		...
		if($data){
			...
			if($data['po_types_indentify'] == 'cfd'){
				$data->avatar = PostImages::where('posts_id', $id)->get();
			} else {
	...
	public function create($request)
	{
		...
		try {
			$posts = Posts::create([
				...
				'compare_price'=>isset($input['compare_price']) ? $input['compare_price'] : '',
			...
			if ($posts) {
				...
				if (!empty($input['avatar']) && $input['po_types_indentify'] == 'cfd') {
					// $input['avatar'] = $this->webHelper->_uploadImage($input['avatar'], 'project_uploads');
					foreach ($input['avatar'] as $key => $value) {
						$file = $this->webHelper->_uploadImage($value['file'], 'project_uploads', true);
						PostImages::create([
							'posts_id'=>$posts->id,
							'url'=>$file
						]);
		  ...
		  //       			'options_indentify'=>$value
				...
				// 			$value['image'] = $this->webHelper->_uploadImage($value['image'], 'project_uploads');
				// 		} else {
				// 			$value['image'] = '';
				// 		}
				// 		Products::create([
				...
				// 			if(!in_array($propertieskey, ['price', 'check', 'compare_price', 'image']) && !strpos($propertieskey, 'label')){
	...
	public function update($request, $id)
	{
		...
		try {
			...
			// 	$input['avatar'] = $this->webHelper->_uploadImage($input['avatar'], 'project_uploads', true);
			// }
			$posts = Posts::where('id', $id)->update([
				// 'po_cates_id'=>!isset($input['category']) ? '' : $input['category'],
				'pages_id'=>$input['pages_id'],
				// 'avatar'=>$input['avatar'],
				'price'=>isset($input['price']) ? $input['price'] : '',
				'compare_price'=>isset($input['compare_price']) ? $input['compare_price'] : '',
				// 'manufacturers'=>!isset($input['manufacturers']) ? '' : $input['manufacturers'],
				'active'=>$input['active']
			]);
			if ($posts) {
				...
				if($input['specifications'] && count($input['specifications']) > 0 && isset($input['category']) && $input['category']) {
				...
				// 	foreach ($input['variations'] as $key => $value) {
				// 		if(isset($value['id'])){
				// 			$product = Products::where('posts_id', $id)->where('sku', $value['sku'])->first();
				// 			if (!empty($value['image']) && $product->avatar != $value['image']) {
				// 				$value['image'] = $this->webHelper->_uploadImage($value['image'], 'project_uploads');
				// 			}

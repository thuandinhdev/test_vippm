...
class PageController extends Controller
{
	...
	{
		$builControlPage = $this->pageRepo->builControlPage();

		if ($builControlPage) {
			if(isset($builControlPage['status']) && isset($builControlPage['msg']) && !$builControlPage['status']){
				return response()->json(['error' => $builControlPage['msg']], 400);
			} else {
				return response()->json($builControlPage);
			}
		} else {
			return response()->json(['error' => 'Manager Widget has not been found.'], 400);
		}

...
class FontendController extends BaseController{
	...
	// 	$APP_URL = 'http://localhost/web-food-demo/public/';
		...
		if($path_string == 'home-control'){
			$rootDataForPage = ['file_path'=>$file_path, 'resource_path'=>$resource_path, 'public_path'=>$public_path];
				return view('home-control', $rootDataForPage);
		}
		...
		// if($path_string == 'view-stl-file'){
		// 	return view('view-stl-file', ['file_path'=>$file_path, 'resource_path'=>$resource_path, 'public_path'=>$public_path]);
		// }
		...
		switch ($pageData->type) {
			...
			case 3:
				...
				$data->properties =  Properties::join('web_options', 'web_options.indentify', 'web_pro_properties.options_indentify')->join('web_options_translation', 'web_options_translation.options_id', 'web_options.id')->where('web_pro_properties.posts_id', $pageData->pages_id)->select('web_options_translation.name as options_name', 'web_pro_properties.options_indentify', 'web_options.avatar')->get();
				foreach ($data->properties as $key_properties => $value_properties) {
					$data->properties[$key_properties]->propertiesValue = OptionsValue::where('options_indentify', $value_properties->options_indentify)->where('options_types_indentify', 'properties')->get();
				}
...
//		return $public_path;
	...
	public function cart() {
		...
		Cart::add(array('id' => 1, 'name' => 'bánh mỳ', 'qty' => 1, 'price' => 20000));
		$cart = Cart::content();
